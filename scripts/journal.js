...
async function loadJournal(){
  ...
  try{
    ...
    if(story){const post=posts.find(p=>p.id===new URLSearchParams(location.search).get("post"));story.replaceChildren();if(!post){story.append(make("h1","This note isn't here."),make("p","It may have moved or hasn't been published yet."));return;}document.title=`${post.title} | Haitish Puran`;const body=make("div",undefined,"story-body");renderBody(post.body,body);story.append(make("p",`${prettyDate(post.date)} / ${Math.max(1,Math.ceil(post.body.split(/\s+/).length/220))} MIN READ`,"micro"),make("h1",post.title),make("p",post.excerpt),body);document.querySelector("#conversation").hidden=false;
      ...
      document.querySelector("#reply-form").addEventListener("submit",event=>{event.preventDefault();const reply=document.querySelector("#reply").value.trim();if(reply)location.href=`mailto:haitish.astro@gmail.com?subject=${encodeURIComponent(`Reply: ${post.title}`)}&body=${encodeURIComponent(`${reply}\n\nRegarding: ${location.href}`)}`;});
    }
